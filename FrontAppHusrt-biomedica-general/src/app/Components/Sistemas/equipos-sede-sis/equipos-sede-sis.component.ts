...
export class EquiposSedesSisComponent implements OnInit {
  ...
  async loadSede() {
    try {
      this.sede = await this.sedeService.getSedeById(this.idSede);
    } catch {
  ...
  async loadEquipos() {
    ...
    this.error = null;
    try {
      const servicios = await this.servicioService.getServiciosBySede(this.idSede);
      ...
      const equiposPorServicio = await Promise.all(
        servicios.map((s: any) =>
          firstValueFrom(this.sysequiposService.getEquipos({ id_servicio_fk: s.id }))
            .then(res => res.success ? (Array.isArray(res.data) ? res.data : [res.data]) : [])
            .catch(() => [] as SysEquipo[])
        )
      );
      this.equipos = equiposPorServicio.flat();
      this.applyFilters();
    } catch {
      this.error = 'Error al cargar los equipos de la sede.';
  ...
  goToPage(page: number) {
    ...
    this.currentPage = page; this.updatePage();
  }

  getPagesArray(): number[] {
    const delta = 2, pages: number[] = [];
  ...
  volverASedes() { this.router.navigate(['/adminsistemas/sedes']); }
  ...
  openReporteForm(equipo: any) {
    ...
    sessionStorage.setItem('origenReporte', '/adminsistemas/equipossede');

...
app.use('/sysreportesmtto', SysReporteMantenimientoRoutes, checkToken);
...
async function runMigrations() {
  try {
    const [cols] = await sequelize.query("SHOW COLUMNS FROM `SysReporteMantenimiento` LIKE 'tipo_mantenimiento'");
    if (cols.length === 0) {
      await sequelize.query('DROP TABLE IF EXISTS `SysReporteMantenimiento`');
      console.log('[migration] SysReporteMantenimiento dropped — will be recreated with current schema');
    }
  } catch (_) {

...
const USERNAME = 'sys.tecnico';
const PASSWORD = 'Tecnico123*';
const EMAIL = 'tecnico.sistemas@husrt.local';

(async () => {
  try {
    await sequelize.authenticate();

    let rol = await Rol.findOne({ where: { nombre: 'SISTEMASTECNICO' } });
    if (!rol) {
      rol = await Rol.create({ nombre: 'SISTEMASTECNICO' });
      console.log(`Rol SISTEMASTECNICO creado (id=${rol.id})`);
    } else {
      console.log(`Rol SISTEMASTECNICO encontrado (id=${rol.id})`);
    }

    let cargo = await Cargo.findByPk(1);
    if (!cargo) {
      cargo = await Cargo.create({ nombre: 'Técnico' });
    ...
    console.log(`  Rol:       SISTEMASTECNICO`);
    console.log('==================================\n');
